...
public sealed class SupplierPaymentTests
{
    ...
    [Fact]
    public async Task A_payment_by_cheque_raises_one_linked_cheque_for_the_total()
    {
        var (companyId, supplierId, code) = await SeedCompanyAndSupplier();
        var change = new FakeChangeContext { UserId = 1, CompanyId = companyId };
        var inv1 = await SeedSupplierInvoice(companyId, supplierId, code, "SINV-E1", 100m);
        var inv2 = await SeedSupplierInvoice(companyId, supplierId, code, "SINV-E2", 50m);
        ...
        await using (var db = _fixture.CreateContext(change))
        {
            // One cheque, for the whole payment (not one per invoice), linked back to the payment.
            var cheque = await db.Cheques.SingleAsync(c => c.SourceType == ChequeSource.SupplierPayment && c.SourceId == paymentId);
            cheque.Amount.Should().Be(150m);
            cheque.ChequeNumber.Should().Be("600321");
            cheque.Bank.Should().Be("NTB");
        }
    }
